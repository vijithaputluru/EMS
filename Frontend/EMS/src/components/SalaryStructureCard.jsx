...
function SalaryStructureCard({
  ...
  const parsedCtcValue =
    Number(ctcValue);
 
  const currentValue =
    Number.isFinite(parsedCtcValue)
      ? parsedCtcValue
      : SALARY_MIN;
  ...
  const salaryOptions =
    Array.from(
      {
        length:
          ((SALARY_MAX - SALARY_MIN) / 50000) + 1,
      },
      (_, index) =>
        SALARY_MIN + (index * 50000)
    );
 
  const hasExactDropdownValue =
    salaryOptions.includes(
      currentValue
    );
 
  const dropdownOptions =
    hasExactDropdownValue
      ? salaryOptions
      : [
        currentValue,
        ...salaryOptions,
  ...
  const numericSearchQuery =
    normalizedSearchQuery.replace(
      /\D/g,
      ""
    );
 
  const filteredSalaryOptions =
    dropdownOptions.filter(
      (amount, index, options) => {
 
        if (
          options.indexOf(amount) !==
          index
        ) {
          return false;
        }
  ...
  const dropdownId =
    `${idPrefix || "salary"}-dropdown-options`;
 
  const dropdownSearchId =
    `${idPrefix || "salary"}-dropdown-search`;
    ...
    <div className="salary-structure-card">
      ...
      >
        ...
        >
          ...
          <span
            className="salary-dropdown-chevron"
            aria-hidden="true"
          />
