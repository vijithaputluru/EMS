...
function CompanyDetails() {
  ...
  const validateCompany = () => {
    ...
    if (!company.established.trim()) errors.established = "Established Date is required";
    ...
    if (!company.gst.trim()) errors.gst = "GST Number is required";
    if (!company.tin.trim()) errors.tin = "TIN Number is required";
    if (!company.pan.trim()) errors.pan = "PAN Number is required";
  ...
  const validateBranch = () => {
    ...
    } else if (!/^[A-Za-z0-9\s&-]{1,40}$/.test(branch.name)) {
      errors.name = "Use only letters, numbers, spaces, & or -";
    }
 
    if (!branch.established.trim()) errors.established = "Established Date is required";
    ...
    if (!branch.email.trim()) {
      errors.email = "Email Address is required";
    } else if (!isValidEmail(branch.email)) {
      errors.email = "Enter a valid email address";
    }
